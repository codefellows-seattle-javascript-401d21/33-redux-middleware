let initialState = localStorage['expenses']
...
export default (state=initialState, action) => {
  ...
  switch(type) {
    ...
    case 'EXPENSE_CREATE': return {
      ...state, [payload.catId]: [...state[payload.catId], payload]
    };

    case 'EXPENSE_UPDATE': {
      let changedState = {...state};
      let changedExpense = state[payload.catId].map(
    ...
    case 'EXPENSE_DELETE': {
      let changedState = { ...state };
      let changedExpense = state[payload.catId].filter(
